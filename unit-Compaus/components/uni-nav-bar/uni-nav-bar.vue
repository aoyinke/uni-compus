	...
	<view class="uni-navbar">
		<view :class="{ 'uni-navbar--fixed': fixed, 'uni-navbar--shadow': shadow}" :style="{ 'background-color': backgroundColor }"
		 class="uni-navbar__content">
			...
			<view :style="{ color: color,backgroundColor: backgroundColor,height:customBarH + 'px',paddingTop:statusBarH + 'px'}" class="uni-navbar__header uni-navbar__content_view">
				<view @tap="onClickLeft" class="uni-navbar__header-btns uni-navbar__header-btns-left uni-navbar__content_view">
					...
					<view :class="{ 'uni-navbar-btn-icon-left': !leftIcon.length }" class="uni-navbar-btn-text uni-navbar__content_view"
					 v-if="leftText.length">
						<text :style="{ color: color, fontSize: '14px',marginTop:'20upx'}">{{ leftText }}</text>
					</view>
					<slot name="left" />
				</view>
